...
def build_ssd_model(input_shape=(224, 224, 3), num_classes=2):
    ...
    # Classification head (binary classification: 0 or 1)
    class_output = layers.Dense(1, activation='sigmoid', name="class_output")(combined_features)
...
model.compile(
    ...
    loss={
        ...
        'class_output': 'binary_crossentropy'  # Cross-entropy for classification
    },
    metrics={
        'bbox_output': ['mae'],  # Mean absolute error for bounding box
        'class_output': ['accuracy']  # Classification accuracy
    }
)
...
checkpoint_callback = tf.keras.callbacks.ModelCheckpoint(
    filepath=os.path.join(checkpoint_dir, "ssd_model_epoch_{epoch:02d}.keras"),
    # Save model as 'ssd_model_epoch_XX.keras'
