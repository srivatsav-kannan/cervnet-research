...
def read_json_file(json_path, resize_width=224, resize_height=224):
    ...
    oimg = Image.open(img_dir).convert("RGB")

    img = img.resize((224, 224))
    img = tf.keras.preprocessing.image.img_to_array(img)
    result = model.predict(np.array([img]))
    result = np.array(result)[0]
    result = np.reshape(result, (23, 2))
    print(result.shape)

    return pic_id, result
...
def read_json_folder(json_path):
    ...
    json_files = [
        json_file for json_file in json_files if json_file.endswith('.png') or json_file.endswith('.jpg')]
...
if __name__ == '__main__':
    # json_folder_path = '/Users/srivatsavkannan/Datasets/C-Spine Xray/X-ray Atlas/datasets-JSON'  # 您的JSON文件夹路径
    image_folder_path = '/Users/srivatsavkannan/Datasets/CervicalNew10/AllROS'
    output_excel_path = '/Users/srivatsavkannan/Datasets/CervicalNew10/results_ros2.xlsx'  # 您想要保存结果的Excel文件路径
    cal_json_folder(image_folder_path, output_excel_path)
